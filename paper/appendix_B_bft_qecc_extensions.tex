...
This is an analytic channel condition. It is not required for the finite OPH normal-form theorem,
where termination follows from strict Lyapunov descent of accepted repairs.
\end{proposition}

\begin{proposition}[Spectral-gap criterion {\normalfont\small [Model-dependent]}]
\label{conj:spectral-gap}
If the transfer operator $\mathcal{T}$ for iterated application of
$\mathcal{R}_{\sigma,\mathcal{N}}$ has a positive spectral gap
$\delta>0$, then the corresponding analytic channel model has exponential convergence. The
finite OPH repair package supplies termination by Lyapunov descent on its declared finite state
space; a spectral gap is a separate quantitative mixing condition for this BFT/QECC-style
extension.
\end{proposition}
...
Under Proposition~\ref{conj:spectral-gap} with gap $\delta>0$,
\[
...
\begin{claim}[Code distance and min-cut
  {\normalfont\small [Assumption-dependent, tightened]}]
...
(i)--(iii). This is a separate topological-code realization condition rather than a
generic theorem about arbitrary overlap graphs.
\end{claim}
...
\begin{theorem}[QECC Correspondence
  {\normalfont\small [Assumption-dependent / Conjecture]}]
\label{thm:qecc}
Suppose conditions (i)--(iii) of Claim~\ref{claim:qecc-mincut} hold.
Then:
\begin{enumerate}[(i)]
  \item \textrm{[Assumption-dependent]} Code distance $d =
...
\begin{itemize}
  ...
  \item Topological-code distance equals graph min-cut only after a concrete primal/dual
        complex, boundary condition, and logical-operator dictionary are supplied for the
        chosen code realization.
