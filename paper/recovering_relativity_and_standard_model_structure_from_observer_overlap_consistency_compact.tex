...
Step 3: scaling-limit cap-pair extraction. Theorem~\ref{thm:geosubnet} fixes the target algebra as the overlap-generated prime geometric subnet, with overlap-trivial auxiliary/record factors removed. The transported cap marginals are compared only on support-visible observables. The regularized modular transport result gives convergent local matrix elements on every fixed collar model, while the fixed finite-dimensional state spaces and the consensus refinement mechanism supply local weak-\(*\) limit states. The GNS construction applied to those compatible limits gives \((\mathcal A_\infty^{\mathrm{geo,sv}}(C),\omega_\infty^{\mathrm{geo},C})\).

Step 4: geometric identification on the prime subnet. Normalize \(C\) to the upper half-plane by a M\"obius transformation and send an ordered null cut-pair on \(\partial C\) to \((0,\infty)\). A M\"obius transformation preserving the cap and that ordered pair acts as \(z\mapsto e^s z\), so ordered cut-pair rigidity collapses the residual cap-preserving conformal freedom to the unique hyperbolic one-parameter subgroup up to normalization. Therefore there is a constant \(\kappa_C>0\) such that
\[
